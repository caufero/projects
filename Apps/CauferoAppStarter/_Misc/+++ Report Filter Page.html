...
<head>
  ...
  <title>Build Report Filter Page</title>
</head>
<body>
            ...
            <div class="nav-group-body">
              ...
              <div class="nav-subgroup open" data-label="AI Prompts Development">
                ...
                <div class="nav-subgroup-body">
                  ...
                  <a class="nav-item sub active" href="https://caufero.github.io/projects/Apps/CauferoAppStarter/_Misc/+++ Report%20Filter%20Page.md" data-label="AI Prompts Development Build Report Filter Page"><span class="dot"></span><span class="nav-text">Build Report Filter Page</span></a>
        ...
        <div class="content-inner">
          ...
          <section class="hero">
            ...
            <div class="hero-inner">
              <div class="hero-top">
                <div class="hero-title">Build Report Filter Page</div>
              ...
              <div class="hero-meta">
                ...
                <span class="chip copper">Report Filter</span>
                <span class="chip amber">FileMaker</span>
                <span class="chip teal">JavaScript</span>
                <span class="chip violet">Flatpickr</span>
              </div>
            </div>
          </section>
          ...
          <section class="card section" id="prompt-section">
            ...
            <div class="opt-toggle">
              <button class="opt-btn active" id="btnOpt1" type="button" onclick="setOption(1)">
                ...
                <div class="opt-text">
                  <div class="opt-label">Named Report</div>
                  <div class="opt-sub">You know the report page name</div>
                </div>
              </button>
            ...
            <div class="opt-desc-strip" id="optDescStrip">
              ...
              <span><strong>Option 1 — Named Report:</strong> Replace <code>[REPORT_NAME]</code> with the name of your target report (e.g., <code>Billed Items Report</code>).</span>
            </div>
          ...
          <section class="card section" id="fm-script-section">
            ...
            <div class="fm-script-wrap">
              <textarea class="fm-script-box" id="fmScriptBox" readonly spellcheck="false">Set Variable [ $$App Section ; Value: "Billed Items Report" ]
#
# (Optional) Keep these if you still want them available in the UI (even if not used in SQL conditions)
Perform Script [ Specified: From list ; "Get Value List" ; Parameter: "Dropdown|Payment Options" ]
Set Variable [ $Payment Options HTML ; Value: Get ( ScriptResult ) ]
#
Perform Script [ Specified: From list ; "Get Value List" ; Parameter: "Dropdown|Patient Visit Reasons|" & $Reason ]
Set Variable [ $Reasons Dropdown HTML ; Value: Get ( ScriptResult ) ]
#
Perform Script [ Specified: From list ; "Get Value List" ; Parameter: "Dropdown|Bill Types" ]
Set Variable [ $Bill Type Options HTML ; Value: Get ( ScriptResult ) ]
#
Set Variable [ $Generate Report Script ; Value: "+++ Billed Items Report" ]
#
#
# ******************************************
# ******************************************
...
#
# CSS
Perform Script [ Specified: From list ; "🖌️ Use Details CSS" ]
        ...
        <div class='rf-topbar'>
          <div class='rf-titleblock'>
            ...
            <div style='min-width:0;'>
              <h1 class='rf-title'>Billing Items Report</h1>
              <div class='rf-subtitle'>Pick your filters, preview what will be generated, then run the report.</div>
            </div>
          </div>
          ...
          <div class='rf-card-body'>
            ...
            <div class='rf-section'>
              ...
              <div class='rf-form-grid'>
                ...
                <!-- NEW: Bill Type (THIS ONE MATCHES YOUR SQL PLACEHOLDER FILTER) -->
                <div class='rf-field'>
                  <label for='bill_type'>Bill Type</label>
                  <select id='bill_type' onchange=\"updatePreview(); saveDraft();\">
                    " & $Bill Type Options HTML & "
                  </select>
                </div>

                <!-- Optional: keep these (not part of Billing Items SQL you shared) -->
                <div class='rf-field'>
                  <label for='payment_option'>Payment Option</label>
                  <select id='payment_option' onchange=\"updatePreview(); saveDraft();\">
                ...
                <div class='rf-field'>
                  ...
                  <select id='reason' onchange=\"updatePreview(); saveDraft();\">
                    <option value=''>All Reasons</option>
                    " & $Reasons Dropdown HTML & "
                  </select>
                </div>
            ...
            <div class='rf-preview'>
              <h4>Preview</h4>
              <div class='rf-kv'>
                <div class='rf-k'>Date</div>
                <div class='rf-v' id='preview_date'>-</div>

                <div class='rf-k'>Bill Type</div>
                <div class='rf-v' id='preview_bill_type'>All</div>

                <div class='rf-k'>Payment Option</div>
                <div class='rf-v' id='preview_payment_option'>All</div>

                <div class='rf-k'>Visit Reason</div>
                <div class='rf-v' id='preview_reason'>All</div>
              </div>
            </div>
...
"
  ...
  function readFilters() {
    ...
    const billType = (document.getElementById('bill_type') || {}).value || 'All';
  ...
  function validateFilters(filters) {
    const hasAnyDate =
      !!filters.dateSingleReadable ||
      !!filters.dateFromReadable ||
      !!filters.dateToReadable;
    ...
    if (!hasAnyDate && !hasAnyOther) {
      alert('Please provide at least one filter: Date, Bill Type, Payment Option, or Visit Reason.');
      return false;
    }

    if (filters.mode === 'range') {
      if ((filters.dateFromReadable && !filters.dateToReadable) || (!filters.dateFromReadable && filters.dateToReadable)) {
    ...
    if (!filters.dateSingleReadable && hasAnyOther) return true;

    if (!filters.dateSingleReadable && !hasAnyOther) {
...
"
  ...
  function updatePreview() {
    ...
    const previewBillType = document.getElementById('preview_bill_type');
    const previewPayment = document.getElementById('preview_payment_option');
    ...
    if (previewDate) {
      if (f.mode === 'range') {
        ...
        previewDate.textContent = a + ' to ' + b;
      } else {
        previewDate.textContent = f.dateSingleReadable || '-';
      }
    }

    if (previewBillType) previewBillType.textContent = f.billType || 'All';
    if (previewPayment) previewPayment.textContent = f.paymentOption || 'All';
    if (previewReason) previewReason.textContent = f.reason || 'All';
  }
" ]
...
"
  ...
  function saveDraft() {
    try {
      const f = readFilters();
      localStorage.setItem('billing_items_report_filters', JSON.stringify(f));
    } catch (e) {
      /* ignore */
    }
  }
  ...
  function loadDraft() {
    try {
      const raw = localStorage.getItem('billing_items_report_filters');
      if (!raw) return;
      ...
      const payment = document.getElementById('payment_option');
      const reason = document.getElementById('reason');

      if (billType) billType.value = f.billType || 'All';
      if (payment) payment.value = f.paymentOption || '';
      if (reason) reason.value = f.reason || '';
...
"
  ...
  function clearFilters() {
    ...
    if (billType) billType.value = 'All';
...
"
  ...
  function buildBillingItemsCondition(from, to, billType, reason, paymentOption) {
    const fromDMY = mysqlToDMY(from);
    const toDMY = mysqlToDMY(to);

    const bt = (billType || \"All\").trim();
    const rs = (reason || \"\").trim();
    const pay = (paymentOption || \"\").trim();

    const btQ = sqlQuoted(bt);
    const rsQ = sqlQuoted(rs);
    const payQ = sqlQuoted(pay);

    return [
      `AND t1.\\\"Date Scheduled\\\" >= '${fromDMY}'`,
      `AND t1.\\\"Date Scheduled\\\" <= '${toDMY}'`,

      `AND (`,
      `  ${btQ} = 'All'`,
      `  OR ( ${btQ} = 'Lab' AND t7.ID IS NOT NULL )`,
      `  OR ( ${btQ} = 'Pharmacy' AND t4.ID IS NOT NULL )`,
      `  OR ( ${btQ} = 'Service' AND t5.ID IS NOT NULL )`,
      `  OR ( ${btQ} = 'Consumable' AND t9.\\\"Category\\\" = 'Consumable' )`,
      `  OR ( ${btQ} = 'Other' AND t7.ID IS NULL AND t4.ID IS NULL AND t5.ID IS NULL AND t9.ID IS NULL )`,
      `)`,

      `AND ( ${rsQ} IS NULL OR ${rsQ} = '' OR ${rsQ} = 'All' OR t1.\\\"Reason\\\" = ${rsQ} )`,
      `AND ( ${payQ} IS NULL OR ${payQ} = '' OR ${payQ} = 'All' OR tPay.\\\"Option\\\" = ${payQ} )`
    ].join(' ');
  }
  ...
  function generateReport() {
    ...
    let single = getMySQLFormattedDateFromReadable(filters.dateSingleReadable);
    let from = getMySQLFormattedDateFromReadable(filters.dateFromReadable);
    let to = getMySQLFormattedDateFromReadable(filters.dateToReadable);

    if (filters.mode !== 'range' && single) { from = single; to = single; }

    const anyDateIntent = !!single || !!from || !!to;
    if (anyDateIntent && !to) {
      const d = new Date();
    ...
    const billType = (filters.billType || \"All\").trim();
    const reason = (filters.reason || \"\").trim();
    const paymentOption = (filters.paymentOption || \"\").trim();

    const safe = (v) => String(v || '').replaceAll('|', '/').trim();

    const conditionSQL = buildBillingItemsCondition(from, to, billType, reason, paymentOption);

    const previewDetails =
      `PreviewDetails: Period: ${safe(from)} to ${safe(to)} - Bill Type: ${safe(billType || 'All')} - Reason: ${safe(reason || 'All')} - Payment: ${safe(paymentOption || 'All')}`;
...
# ******************************************
#
Perform Script [ Specified: From list ; "WebDirect Pause" ]
Refresh Window [ Flush cached join results ]</textarea>
            </div>
          </section>
            ...
            <div class="attach-grid single">
              <a class="attach-item" href="https://caufero.github.io/projects/_Misc/_/AI%20Prompts/Get%20Value%20List.md">
                ...
                <div>
                  ...
                  <div class="attach-desc">Required if the report filter page uses any value list dropdowns.</div>
                </div>
      ...
      <aside class="rail rail-right" id="railRight" aria-label="Right panel">
        ...
        <div class="rail-body">
          ...
          <div class="panel-card">
            ...
            <div class="stack">
              <div class="stack-item">The reference FileMaker report filter script is displayed in a section below</div>
              <div class="stack-item">Copy the prompt, then copy the FM script separately</div>
              <div class="stack-item">Choose Option 1 if you know the report name, Option 2 if you have a SQL query</div>
              <div class="stack-item">The AI returns a complete, updated FM report filter script</div>
            </div>
          </div>
          ...
          <div class="panel-card">
            ...
            <div class="stack">
              <div class="stack-item">
                ...
                <div><strong>Named Report</strong> — you know what report page this filter will drive. The AI infers the relevant fields and filter conditions from context.</div>
              </div>
              <div class="stack-item">
                <span class="mini-pill teal" style="margin-bottom:6px;display:inline-flex;">OPT 2</span>
                <div><strong>Custom SQL</strong> — paste your full ExecuteSQL statement. The AI derives filter fields directly from your WHERE conditions and table aliases.</div>
              </div>
            </div>
          </div>
          ...
          <div class="panel-card">
            ...
            <div class="pill-row">
              <span class="pill2">Option 1: report name</span>
          ...
          <div class="panel-card">
            ...
            <div class="pill-row">
              <span class="pill2">Report Filter Page</span>
          ...
          <div class="panel-card">
            ...
            <div class="side-grid">
              ...
              <a class="side-pill" href="https://caufero.github.io/projects/_Misc/_/AI%20Prompts/Build%20List%20Page.md">Build List Page</a>
          ...
          <div class="panel-card">
            ...
            <div class="stack">
              ...
              <div class="stack-item">2. Edit the placeholder directly in the textarea — replace <code style="font-size:11px;">[REPORT_NAME]</code> or <code style="font-size:11px;">[FULL_EXECUTESQL_STATEMENT]</code></div>
  ...
  <script>
    (function(){
      ...
      /* ---- Prompt Templates ---- */
      var OPT1_TEMPLATE = [
        'I want you to study the FileMaker script below. It\'s a new FileMaker script that was written to generate a filter page for a report.',
        '',
        'I want you to update the new filter page (FileMaker script below which is what I\'m using now to generate the filter page) so it generates the [REPORT_NAME] report page (the actual report is being handled by another script).',
        '',
        '**Note:**',
        '- The filters should be based on the where conditions used to generate the report',
        ...
        '',
        '[Paste the reference FileMaker script here]'
      ].join('\n');

      var OPT2_TEMPLATE = OPT1_TEMPLATE.replace(
        'so it generates the [REPORT_NAME] report page',
        'so it generates a report (the actual report is being handled by another script) whose recordset is generated with this sql:\n\n[FULL_EXECUTESQL_STATEMENT]\n'
      );
      ...
      function setOption(n) {
        ...
        if (descStrip) {
          ...
          if (n === 1) {
            descStrip.innerHTML = ico + '<span><strong>Option 1 \u2014 Named Report:</strong> Replace <code>[REPORT_NAME]</code> with the name of your target report (e.g., <code>Billed Items Report</code>).</span>';
          } else {
            descStrip.innerHTML = ico + '<span><strong>Option 2 \u2014 Custom SQL:</strong> Replace <code>[FULL_EXECUTESQL_STATEMENT]</code> with your full ExecuteSQL query so the AI can derive the correct filters and condition builder from your WHERE conditions.</span>';
          }
        }
      }
      ...
      function highlightKeywords() {
        ...
        if (!main) return;
        var keywords = ['CauferoAppStarter', 'FileMaker', 'Report Filter', 'ExecuteSQL', 'generateReport', 'mysqlToDMY', 'Value List', 'Flatpickr', 'buildCondition'];
