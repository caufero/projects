...
<head>
  ...
  <title>AgentMail — Caufero Workspace</title>
</head>
<body>
      ...
      <div class="topbar-inner">
        ...
        <div class="topbar-center">
          <div class="page-chip">
            ...
            <span class="page-chip-text">AgentMail</span>
          </div>
        </div>
    ...
    <div class="shell" id="shell">
      ...
      <aside class="rail rail-left" id="railLeft" aria-label="Side menu">
        ...
        <nav class="rail-body" id="navTree">
          ...
          <a class="nav-item active" href="https://caufero.github.io/projects/_Misc/_/Netflix.md#" data-label="Netflix">
            <span class="nav-ico" aria-hidden="true"><svg viewBox="0 0 24 24" class="ico sm" fill="currentColor" aria-hidden="true"><path d="M6 4h4l4 10V4h4v16h-4l-4-10v10H6z"></path></svg></span>
      ...
      <!-- CENTER: video fills the full remaining width (right rail removed) -->
      <main class="content" id="content" role="main">
        <div class="content-inner">
          <section class="video-card card">
            <div class="video-wrap">
              <video
                src="some_path/video.mp4"
                controls
                preload="metadata"
              ></video>
            </div>
  ...
  <script>
    (function(){
      ...
      function filterNav(q){
        ...
        navTree.querySelectorAll("[data-label]").forEach(function(el){
          var hit=!query||(el.getAttribute("data-label")||"").toLowerCase().indexOf(query)!==-1;
          el.classList.toggle("is-hit",hit); el.classList.toggle("is-dim",!!query&&!hit);
        });
        navTree.querySelectorAll(".nav-group,.nav-subgroup").forEach(function(g){
          var anyHit=[].slice.call(g.querySelectorAll("[data-label]")).some(function(e){return e.classList.contains("is-hit");});
          g.classList.toggle("is-dim",!!query&&!anyHit);
          if(query&&anyHit)g.classList.add("forced-open"); else g.classList.remove("forced-open");
        });
      }

      function wireDisclosure(){
        document.addEventListener("click",function(e){
          var h=closest(e.target,"[data-toggle='group'],[data-toggle='subgroup']");
          if(!h)return; h.parentNode&&h.parentNode.classList.toggle("open");
        });
      }
      ...
      function closeMobileOnOutside(e){
        ...
        if(!railLeft)return;
        if(!closest(e.target,"#railLeft")&&!closest(e.target,"#btnMenu")) railLeft.classList.remove("open");
      }

      function scrollNavToActive(){
        var a=navTree&&navTree.querySelector(".nav-item.active"); if(!a)return;
        var el=a.parentNode;
        while(el&&el!==navTree){ if(el.classList&&(el.classList.contains("nav-group")||el.classList.contains("nav-subgroup")))el.classList.add("open"); el=el.parentNode; }
        var rb=document.querySelector("#railLeft .rail-body");
        if(rb) setTimeout(function(){ rb.scrollTop=a.offsetTop-rb.clientHeight/2+a.offsetHeight/2; },60);
      }
      ...
      function init(){
        ...
        if(btnMenu)  btnMenu.addEventListener("click",function(){
          if(window.matchMedia("(max-width:980px)").matches) railLeft&&railLeft.classList.toggle("open");
          else document.body.classList.toggle("compact-rails");
        });
        if(navClear)  navClear.addEventListener("click",function(){ navSearch.value=""; filterNav(""); navSearch.focus(); });
  ...
  <style>
    :root{
      --slate-900:#0f172a; --slate-600:#475569; --slate-500:#64748b; --slate-100:#f1f5f9; --slate-50:#f8fafc;
      --caufero-800:#0e3344; --teal-700:#0f5e5b; --teal-600:#147a74; --teal-500:#1a9a93;
      --violet-600:#5b49c8; --amber-600:#b7791f; --copper-600:#d8743d; --rose-700:#b4233a;
      --bg:var(--slate-50);
      --surface:rgba(255,255,255,.88); --surface2:rgba(255,255,255,.78);
      ...
      --r-xl:18px; --topbarH:56px; --railW:288px; --railWCompact:228px;
      --focus:rgba(14,51,68,.22);
    }
    *{box-sizing:border-box;}
    html,body{height:100%;}
    body{margin:0;font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;background:var(--bg);color:var(--slate-900);line-height:1.45;overflow-x:hidden;-webkit-font-smoothing:antialiased;}
    ...
    .page[data-theme="dark"]{--bg:#070a12;--surface:rgba(10,14,26,.82);--surface2:rgba(10,14,26,.68);--line:rgba(148,163,184,.14);color:var(--slate-100);}
    ...
    .topbar-inner{height:100%;display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:12px;padding:0 14px;}
    ...
    /* Shell — left rail + content only, right rail removed */
    .shell{padding-top:var(--topbarH);display:grid;grid-template-columns:var(--railW) 1fr;min-height:100vh;gap:14px;padding-left:14px;padding-right:14px;}
    body.compact-rails .shell{grid-template-columns:var(--railWCompact) 1fr;}
    ...
    .rail-top{padding:14px 14px 12px;border-bottom:1px solid var(--line);background:linear-gradient(180deg,rgba(255,255,255,.65),rgba(255,255,255,0));}
    ...
    .nav-subgroup-head{padding-left:12px;}
    ...
    /* Content */
    .content{min-width:0;position:sticky;top:calc(var(--topbarH) + 14px);height:calc(100vh - var(--topbarH) - 28px);overflow:hidden;padding:0;}
    ...
    /* Video card — fills entire content column */
    .video-card{flex:1 1 0;min-height:0;overflow:hidden;padding:0;border-radius:var(--r-xl);background:#000;}
    .video-wrap{width:100%;height:100%;display:flex;align-items:center;justify-content:center;border-radius:var(--r-xl);overflow:hidden;background:#000;}
    .video-wrap video{width:100%;height:100%;object-fit:contain;display:block;border-radius:var(--r-xl);}
    ...
    @media (max-width:980px){
      ...
      .content{position:static;height:calc(100vh - var(--topbarH) - 28px);padding:14px 0;}
