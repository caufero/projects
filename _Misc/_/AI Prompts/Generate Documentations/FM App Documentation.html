...
<head>
  ...
  <title>FM App Documentation Prompt</title>
</head>
<body>
      ...
      <div class="topbar-inner">
        ...
        <div class="topbar-center">
          <div class="page-chip">
            ...
            <span class="page-chip-text">AI Prompts › Docs</span>
          </div>
        </div>
        ...
        <nav class="rail-body" id="navTree">
          ...
          <div class="nav-group open" data-label="AI Prompts">
            ...
            <div class="nav-group-body">
              ...
              <div class="nav-subgroup open" data-label="AI Prompts Docs">
                <button class="nav-subgroup-head" type="button" data-toggle="subgroup">
                ...
                <div class="nav-subgroup-body">
                  ...
                  <a class="nav-item sub active" href="https://caufero.github.io/projects/_Misc/_/AI%20Prompts/Generate%20Documentations/FM%20App%20Documentation.md" data-label="AI Prompts Docs Documentation For A FileMaker Application"><span class="dot"></span><span class="nav-text">Documentation For A FileMaker Application</span></a>
                </div>
              </div>
        ...
        <div class="content-inner">
          <section class="hero">
            ...
            <div class="hero-inner">
              <div class="hero-top">
                <div class="hero-title">FM App Documentation Prompt</div>
                <div class="hero-sub">Generate exhaustive developer-grade documentation from a FileMaker DDR XML export. Covers data model, scripts, layouts, security, integrations, and refactoring opportunities.</div>
              </div>
              ...
              <div class="hero-meta">
                ...
                <span class="chip teal">DDR XML</span>
                <span class="chip amber">Documentation</span>
              </div>
            </div>
          </section>

          <section class="card section" id="steps-section">
            <div class="section-head">
              <div class="section-title">
                <span class="section-emoji" aria-hidden="true">📌</span>
                <span>Steps</span>
                <span class="section-sub">Run these first.</span>
              </div>
            </div>
            <div class="steps-body">
              <ol class="steps-list">
                <li class="step-item">Generate XML of the FileMaker Application.</li>
                <li class="step-item">This generates a file named <code class="inline-code">&lt;The_App_Name.xml&gt;</code>.</li>
                <li class="step-item">Use the prompt below to generate documentation on the application.</li>
              </ol>
            </div>
          </section>
          ...
          <section class="card section" id="prompt-section">
            <div class="section-head">
              <div class="section-title">
                <span class="section-emoji" aria-hidden="true">📋</span>
                <span>Prompt</span>
                <span class="section-sub">Upload DDR XML, then paste this.</span>
              </div>
            ...
            <div class="prompt-wrap">
              <textarea class="prompt-box" id="promptBox" readonly spellcheck="false">I've uploaded a FileMaker Database Design Report (DDR) exported as XML from a real solution. Your job is to thoroughly analyze the XML and produce a complete, well-structured, developer-grade documentation of the entire solution.
Hard Rules:
1) Use ONLY the content in the uploaded DDR XML. Do not invent tables, fields, scripts, layouts, or business logic that are not present in the XML.
2) If something is unclear in the DDR, mark it clearly as "Unknown from DDR" and list what evidence is missing.
3) Be exhaustive. Cover every table, relationship, script, layout, value list, privilege set, custom function, and external reference described by the DDR.
4) Keep language clear, specific, and implementation-oriented. No fluff.
5) Output must be in Markdown (ready to paste into a docs repo).
Deliverables:
Produce a documentation pack with these sections, in this exact order:
A) Executive Overview
- What this solution appears to do (business domain inferred from table names, layout names, script names).
- High-level architecture: core modules, main entities, and how they connect.
- Key workflows inferred from scripts + layouts.
B) System Map
- List major modules/features and which tables/layouts/scripts belong to each.
- Provide a short "system mental model" of how a developer should think about the file.
C) Data Model Documentation
For EACH table occurrence / base table:
- Table name (and whether it's base table vs TO)
- Primary key pattern (how IDs look, auto-enter calc hints, UUID usage if present)
- Fields list (grouped by type: text/number/date/timestamp/container/calculation/summary)
- For each field: name, type, auto-enter behavior, validation, storage options, indexing hints if present
- Any obvious conventions (prefixes like z_, g_, $$, etc.)
D) Relationships Graph
- Explain relationships as a readable graph:
  - TO → TO links
  - Match fields
  - Join type hints (if DDR indicates)
  - Cardinality inferred where possible (1-to-many, many-to-many)
- Identify:
  - "Hub tables"
  - "Bridge/join tables"
  - Any relationship chains used for UI context
- Call out risky patterns:
  - Cartesian products
  - Multiple TOs pointing to same base table without naming discipline
  - Overloaded relationships used for filtering
E) Scripts Documentation (Deep)
For EACH script:
- Script name
- Purpose (inferred from steps, called scripts, and related tables/layouts)
- Inputs/parameters:
  - Any Get ( ScriptParameter ), JSON patterns, $$ globals, context assumptions
- Outputs:
  - Set Field, Set Variable results, window navigation outcomes, exports, prints, emails, integrations
- Step-by-step outline (group steps into logical blocks)
- Dependencies:
  - Which tables, layouts, custom functions, value lists it uses
  - Which scripts it calls and which scripts call it (build a call graph)
- Error handling patterns:
  - Get ( LastError ), OnError capture, custom dialogs, commits/rollbacks
- Performance flags:
  - loops, ExecuteSQL, large found sets, server-side vs client-side, "danger steps"
Also include:
- Script Call Graph (top-down)
- "Critical scripts" shortlist (scripts that seem central to everything)
F) Layout &amp; UX Documentation
For EACH layout:
- Layout name, base table occurrence, layout type (form/list/report/dashboard), device intent if inferable
- Purpose and key actions (buttons mapped to scripts if DDR indicates)
- Key fields shown
- Navigation pattern:
  - how user moves through the system
- Reporting layouts:
  - sorting, grouping, subsummary parts, print settings (if DDR reveals)
G) Security Model
- Accounts / Auth model hints (if in DDR)
- Privilege sets:
  - list each privilege set and summarize what it can do (tables, records, layouts, scripts)
- Extended privileges:
  - fmwebdirect, fmapp, fmxml, fmrest, etc if present
- File access / external file references:
  - any external sources and how they're used
H) Value Lists &amp; Custom Functions
For EACH value list:
- Name, source (custom values vs field-based vs relationship-based)
- Where it's used (fields/layouts/scripts)
For EACH custom function:
- Name, purpose, parameters, return shape
- Identify "utility" functions used everywhere
- Identify any JSON utility patterns
I) Integrations &amp; External Dependencies
- Insert from URL usage, cURL options, OAuth, Data API, SMTP, webhooks, JS in web viewers, etc (only if present in DDR)
- Any external file references, plugin hints, or scheduled scripts
...
- List issues found from DDR structure:
  - duplicated logic, god scripts, unclear naming, relationship sprawl, circular dependencies
- Provide concrete refactor suggestions:
  - script modularization
  - TO naming cleanup
  - standard parameter patterns
  - performance quick wins
L) "How To Work With This Solution" Guide
- For a new developer:
  - where to start
  - what not to break
  - how to add a new module safely
  - how to add a new table + UI + scripts following existing conventions
Output Format Rules:
- Use clear headings (H1/H2/H3).
- Use bullet lists and tables where it improves readability.
- Include cross-references:
  - "This layout calls script X"
  - "This script writes to table Y"
- If the DDR is huge, produce the documentation in ordered parts:
  Part 1: Overview + Data Model
  Part 2: Relationships + Scripts (first half)
  Part 3: Scripts (second half) + Layouts
  Part 4: Security + Value Lists + Custom Functions + Integrations + Risks + Working Guide
First Step When XML Arrives:
1) Give me a quick inventory summary:
- number of tables
- number of TOs
- number of scripts
- number of layouts
- number of value lists
- number of privilege sets
- number of custom functions
2) Then begin Part 1 immediately.
Important:
Do not ask me to "describe the solution." The DDR is the source of truth. Only ask questions if the DDR is missing a critical section or the XML is malformed.</textarea>
            </div>
          </section>
      ...
      <aside class="rail rail-right" id="railRight" aria-label="Right panel">
        <div class="rail-top">
          ...
          <div class="rail-sub">FM App Documentation</div>
          <div class="rail-actions">
            <a class="rail-btn" href="https://caufero.github.io/projects/_Misc/_/_Stuff.md#">Home</a>
            <a class="rail-btn" href="#steps-section">Steps</a>
        ...
        <div class="rail-body">
          <div class="panel-card">
            <div class="panel-head">
              ...
              <div class="panel-sub">Exhaustive DDR-based documentation</div>
            </div>
            <div class="stack">
              <div class="stack-item">Upload a FileMaker DDR XML export and paste the prompt</div>
              <div class="stack-item">The AI analyzes the XML and produces developer-grade documentation in 12 sections (A–L)</div>
              <div class="stack-item">Only uses data present in the DDR — nothing invented</div>
              <div class="stack-item">If DDR is huge, output is split into 4 ordered parts</div>
            </div>
          </div>

          <div class="panel-card">
            <div class="panel-head">
              <div class="panel-title">Documentation Sections</div>
              <div class="panel-sub">12 deliverable sections A–L</div>
            </div>
            <div class="pill-row">
              <span class="pill2">A) Executive Overview</span>
              <span class="pill2">B) System Map</span>
              <span class="pill2">C) Data Model</span>
              <span class="pill2">D) Relationships Graph</span>
              <span class="pill2">E) Scripts (Deep)</span>
              <span class="pill2">F) Layout &amp; UX</span>
              <span class="pill2">G) Security Model</span>
              <span class="pill2">H) Value Lists &amp; Custom Functions</span>
              <span class="pill2">I) Integrations</span>
              <span class="pill2">J) Conventions &amp; Standards</span>
              <span class="pill2">K) Risks &amp; Refactoring</span>
              <span class="pill2">L) Working Guide</span>
            </div>
          </div>
          ...
          <div class="panel-card">
            ...
            <div class="pill-row">
              ...
              <span class="pill2">Exhaustive</span>
              <span class="pill2">Tables</span>
              <span class="pill2">Relationships</span>
              <span class="pill2">Scripts</span>
              <span class="pill2">Layouts</span>
              <span class="pill2">Privilege Sets</span>
              <span class="pill2">Custom Functions</span>
              <span class="pill2">Call Graph</span>
              <span class="pill2">Refactoring</span>
            </div>
          </div>

          <div class="panel-card">
            <div class="panel-head">
              <div class="panel-title">Output Parts (large DDR)</div>
              <div class="panel-sub">Ordered split for big files</div>
            </div>
            <div class="side-grid">
              <span class="side-pill">Part 1: Overview + Data Model</span>
              <span class="side-pill">Part 2: Relationships + Scripts (first half)</span>
              <span class="side-pill">Part 3: Scripts (second half) + Layouts</span>
              <span class="side-pill">Part 4: Security + VLs + CFs + Integrations + Risks + Guide</span>
            </div>
          </div>
          ...
          <div class="panel-card">
            <div class="panel-head">
              ...
              <div class="panel-sub">Other documentation prompts</div>
            </div>
            <div class="side-grid">
              <a class="side-pill" href="https://caufero.github.io/projects/_Misc/_/AI%20Prompts/Generate%20Documentations/AI_AgentTraining.md">AI Agent Training</a>
            </div>
          </div>
          ...
          <div class="panel-card">
            ...
            <div class="stack">
              <div class="stack-item">1. Generate DDR XML from your FileMaker application</div>
              <div class="stack-item">2. Upload the XML file to the AI</div>
              <div class="stack-item">3. Copy and paste the prompt above</div>
              <div class="stack-item">4. The AI gives an inventory summary, then starts Part 1</div>
              <div class="stack-item">5. Continue until all 4 parts are complete</div>
            </div>
          </div>
        </div>
