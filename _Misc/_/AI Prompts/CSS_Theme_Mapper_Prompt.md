...
<head>
  ...
  <title>CSS Theme Mapper Prompt</title>
</head>
<body>
      ...
      <div class="topbar-inner">
        ...
        <div class="topbar-center">
          <div class="page-chip">
            ...
            <span class="page-chip-text">AI Prompts › CSS</span>
          </div>
        </div>
      ...
      <aside class="rail rail-left" id="railLeft" aria-label="Side menu">
        ...
        <nav class="rail-body" id="navTree">
          ...
          <div class="nav-group open" data-label="AI Prompts">
            ...
            <div class="nav-group-body">
              <div class="nav-subgroup open" data-label="AI Prompts CSS">
                ...
                <div class="nav-subgroup-body">
                  ...
                  <a class="nav-item sub active" href="https://caufero.github.io/projects/_Misc/_/AI%20Prompts/CSS_Theme_Mapper_Prompt.md" data-label="AI Prompts CSS Map CSS To Some Theme"><span class="dot"></span><span class="nav-text">Map CSS To Some Theme</span></a>
                </div>
              </div>
        ...
        <div class="content-inner">
          <section class="hero">
            ...
            <div class="hero-inner">
              <div class="hero-top">
                <div class="hero-title">CSS Theme Mapper Prompt</div>
                <div class="hero-sub">Map one CSS file's colour system onto another using a reference palette. Colours only — layout stays untouched.</div>
              </div>
              ...
              <div class="hero-meta">
                ...
                <span class="chip teal">CSS 1 → CSS 2 palette</span>
                <span class="chip amber">Colours only</span>
              </div>
            </div>
          </section>
          ...
          <section class="card section" id="prompt-section">
            ...
            <div class="prompt-wrap">
              <textarea class="prompt-box" id="promptBox" readonly spellcheck="false">### TASK
You will receive two CSS blocks: **CSS 1 (to edit)** and **CSS 2 (palette + intent reference)**.
### GOAL
Rewrite **CSS 1** so its **entire colour system** matches **CSS 2** and the UI feels **Tailwind-ish**  while staying **inside CSS 2's palette/tokens**.
---
## NON-NEGOTIABLE RULES
1) **Colours only.** Do NOT change any non-colour values (sizes, spacing, layout, typography, radius, shadow offsets/blur/spread, transitions, etc.).
2) **Do NOT delete, rename, reorder, or omit** any selector, class, property, or rule from CSS 1.
3) **Output must be the FULL CSS 1**, rewritten with **only colour-related values changed**.
4) **Change only comments that are related to colour if the colour changes.**
5) **Do NOT rename variables.** Variable names may keep legacy labels (e.g., `--teal-*`) even if CSS 2 uses blue values. Only values change.
6) **No "leftovers."** If a colour appears in CSS 1, you must attempt to map it to CSS 2. You can professionally and creatively apply newer colours out of the CSS 2 colour pallete if it has no sage match in CSS 2
---
## WHAT COUNTS AS "COLOUR VALUES"
You may change ONLY these in CSS 1:
- `color`
- `background`, `background-color` (colour parts only)
- `border-color` and border shorthands (ONLY the colour portion)
- `outline-color`
- `text-decoration-color`
- `fill`, `stroke`
- `caret-color`
- `box-shadow` (ONLY the colour portions; keep offsets/blur/spread unchanged)
- gradients (ONLY change colour stops; keep angles/positions unchanged)
- any `rgba/hsla/hex/hsl/named colour` literals
Everything else must remain byte-for-byte the same.
---
## CREATIVE PALETTE RULE (CSS 2 IS THE BOUNDARY, NOT A CAGE)
CSS 2 defines the allowed colour FAMILY and style. Your primary intent is to stay inside CSS 2's palette, but you are allowed to be creative and professional if you have to go outside it.
Hard rules:
- You may ONLY use colours that already exist in CSS 2:
  - CSS 2 variables, OR
  - CSS 2 raw colour literals (including its rgba tints).
- You must NOT keep any original CSS 1 colour literal.
- If CSS 1 has a colour that does not appear in CSS 2, you MUST replace it using a creative but consistent choice from CSS 2.
How to be creative (still within CSS 2):
- Prefer using CSS 2 variables (`--slate-*`, `--accent`, `--accent-strong`, `--row-hover-bg`, `--row-selected-bg`) instead of hardcoded colours.
- When you need subtle tints for hover/selected/rings/dividers and CSS 2 has no exact token, use the closest existing CSS 2 rgba tint that matches the intent (e.g., reuse `rgba(37, 99, 235, 0.08/0.10/0.18)` if those exist in CSS 2).
- You may "repurpose" CSS 2 tints across components if it improves consistency (example: use the same blue 0.08 tint for multiple hover surfaces).
- Maintain Tailwind-ish contrast: strong primary text, muted secondary text, light borders, soft surfaces
---
## TAILWIND-ISH BEAUTY GUIDELINES
sharp SaaS UI with crisp edges, 1px hairline borders used as structure, low-to-medium border radius (6px to 10px), minimal blur shadows (very subtle, not "soft UI"), strong grid alignment with consistent spacing scale (8px base), clean typography hierarchy (tight line-height, clear weights), fast subtle transitions (150ms to 220ms), outline icons with consistent stroke, high color richness allowed, vibrant accent palette (teal/cyan/indigo/purple/orange options), use colorful gradients in hero and key CTAs, allow tinted card backgrounds (very light brand tints), buttons with solid brand fills plus hover darken, focus rings in brand color (not blue-only), hover states include slight border color shift toward brand, support both light and dark mode with brand-forward accents, avoid "everything gray" look, keep neutrals only for layout and text legibility, keep backgrounds clean but permit colorful sections and highlights, maintain sharp lines even with color by keeping borders and spacing consistent
---
## OUTPUT FORMAT
Return ONLY the full updated CSS 1 in a single code block. No explanations, no notes, no extra text.
---
## INPUTS
CSS 1:
[PASTE CSS 1 HERE]
CSS 2:
[PASTE CSS 2 HERE]</textarea>
            </div>
          </section>
      ...
      <aside class="rail rail-right" id="railRight" aria-label="Right panel">
        <div class="rail-top">
          ...
          <div class="rail-sub">CSS Theme Mapper</div>
        ...
        <div class="rail-body">
          <div class="panel-card">
            <div class="panel-head">
              ...
              <div class="panel-sub">Maps one CSS file's colour system onto another using a reference palette</div>
            </div>
            <div class="stack">
              <div class="stack-item">Takes two CSS inputs: CSS 1 (to edit) and CSS 2 (palette reference)</div>
              <div class="stack-item">Changes ONLY colour-related values — layout, spacing, typography stay untouched</div>
              <div class="stack-item">Outputs the full rewritten CSS 1 in a single code block</div>
              <div class="stack-item">Creative palette mapping allowed within CSS 2's colour family</div>
            </div>
          </div>

          <div class="panel-card">
            <div class="panel-head">
              <div class="panel-title">Non-Negotiable Rules</div>
              <div class="panel-sub">Hard constraints enforced by this prompt</div>
            </div>
            <div class="pill-row">
              <span class="pill2">Colours only</span>
              <span class="pill2">No deletions</span>
              <span class="pill2">Full CSS output</span>
              <span class="pill2">No variable renames</span>
              <span class="pill2">No leftovers</span>
              <span class="pill2">Tailwind-ish feel</span>
            </div>
          </div>

          <div class="panel-card">
            <div class="panel-head">
              <div class="panel-title">Colour Properties Changed</div>
              <div class="panel-sub">What gets mapped</div>
            </div>
            <div class="pill-row">
              <span class="pill2">color</span>
              <span class="pill2">background</span>
              <span class="pill2">border-color</span>
              <span class="pill2">outline-color</span>
              <span class="pill2">fill / stroke</span>
              <span class="pill2">box-shadow colours</span>
              <span class="pill2">gradient stops</span>
              <span class="pill2">caret-color</span>
            </div>
          </div>

          <div class="panel-card">
            <div class="panel-head">
              <div class="panel-title">Other CSS Prompts</div>
              <div class="panel-sub">Related AI prompts</div>
            </div>
            <div class="side-grid">
              ...
              <a class="side-pill" href="https://caufero.github.io/projects/_Misc/_/AI%20Prompts/Beautify%20Markdown%20File.md#">Beautify Markdown</a>
            </div>
          </div>
          ...
          <div class="panel-card">
            ...
            <div class="stack">
              ...
              <div class="stack-item">2. Replace [PASTE CSS 1 HERE] with the CSS you want to retheme</div>
              <div class="stack-item">3. Replace [PASTE CSS 2 HERE] with the reference palette CSS</div>
              <div class="stack-item">4. Paste the prompt into the AI and send</div>
              <div class="stack-item">5. Get back the full CSS 1 with all colours remapped</div>
            </div>
          </div>
        </div>
