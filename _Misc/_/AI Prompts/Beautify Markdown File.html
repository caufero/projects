...
<head>
  ...
  <title>Beautify Markdown File Prompt</title>
</head>
<body>
        ...
        <div class="content-inner">
          <section class="hero">
            ...
            <div class="hero-inner">
              <div class="hero-top">
                <div class="hero-title">Beautify Markdown File Prompt</div>
                <div class="hero-sub">One-time master prompt that converts any Markdown content into a complete, copy-paste-ready Tailwind-ish HTML page for FileMaker WebViewer.</div>
              </div>
              ...
              <div class="hero-meta">
                ...
                <span class="chip teal">Markdown → HTML</span>
                <span class="chip amber">WebViewer</span>
              </div>
            </div>
          </section>
          ...
          <section class="card section" id="prompt-section">
            <div class="section-head">
              <div class="section-title">
                ...
                <span>Prompt</span>
                <span class="section-sub">Paste Markdown content at the end.</span>
              </div>
            ...
            <div class="prompt-wrap">
              <textarea class="prompt-box" id="promptBox" readonly spellcheck="false">ONE-TIME MASTER PROMPT (Markdown -&gt; Tailwind-ish Page Generator) v3
(Fixes: empty side space, fixed-side scroll, stable alignment)
ROLE
You are my "Markdown -&gt; Tailwind-ish Page Generator" for FileMaker WebViewer pages.
GOAL
When I paste any Markdown file content, generate a complete, copy-paste-ready HTML page that matches my reference UI feel (clean, modern, Tailwind-ish, like my To-Dos page sample). It must be scan-friendly, table-friendly, overflow-safe, and it must FILL the available width without huge dead margins.
INPUTS I WILL PROVIDE
1) PAGE_META block (optional, but if present you must use it)
2) The raw Markdown content
OUTPUT REQUIREMENT (NON-NEGOTIABLE)
- Output ONE single code block only.
- The code block language must be html.
- Output must be a complete, standalone HTML document:
  - &lt;!doctype html&gt;, &lt;html&gt;, &lt;head&gt;, &lt;body&gt;
  - One root wrapper div: &lt;div class="page"&gt;...&lt;/div&gt;
  - Inline &lt;style&gt; near the end of the body (inside the HTML), like the reference
  - Optional inline &lt;script&gt; before &lt;style&gt; (only if needed for highlight, side alignment, or behaviors)
- No extra commentary outside the code block.
- No second code block.
- No explanations.
HARD OUTPUT FORMAT RULES (STRICT) (for all code types: HTML, Javascript and CSS)
1) Newlines policy is strict.
   - Zero blank lines anywhere in the HTML.
   - Never output two consecutive newline characters.
   - Between every closing tag and the next opening tag, exactly ONE newline.
   - Inside &lt;style&gt; and &lt;script&gt;, also no blank lines. Single-newline formatting only.
2) Proper indentation throughout.
3) Before output, do a final lint pass and delete any empty or whitespace-only line.
4) If you cannot comply, fix formatting first, then output.
WIDTH + LAYOUT RULES (CRITICAL)
- The page must be fluid and expand to the viewport width.
- Use a full-width centered frame with small safe gutters:
  - .frame width MUST be: width: min(1400px, calc(100vw - 32px)); margin: 0 auto;
  - Never hard-cap to 1200px.
- Use a two-column layout with fixed side width and flexible main:
  - Grid layout: grid-template-columns: minmax(0, 1fr) 340px;
  - Allow side width range 320px to 360px. Default 340px.
- The main column must stretch and fill all remaining space.
- Do NOT create dead space by anchoring the side using "right: calc((100vw - maxWidth)/2 ...)" tricks.
- Side alignment must be anchored to the same centered frame as the main, using ONE of these approved patterns:
  Pattern A (Preferred, simplest):
  - Put &lt;aside class="side-fixed"&gt; inside the grid's right column container (the side slot).
  - Use position: sticky on desktop for alignment stability.
  Pattern B (Fixed side with perfect alignment):
  - Side is position: fixed, but you compute left and width using the side slot bounding rect:
    - On load and on resize, read sideSlot.getBoundingClientRect()
    - sideFixed.style.left = rect.left + "px"
    - sideFixed.style.width = rect.width + "px"
  - Never compute offsets from viewport center; always use the side slot rect.
- On medium widths:
  - Collapse to one column and make the side static below the main.
- On small widths:
  - One column only and no horizontal page scroll.
SIDE PANEL (FIXED + INTERNAL SCROLL)
The side panel must always exist.
On desktop (wide layout):
- The side panel must NOT move with the main content.
- The side panel must scroll INSIDE itself when content is long.
- Use these required constraints (the core fix):
  - max-height: calc(100vh - 104px) (or equivalent)
  - overflow: auto; overflow-x: hidden; overscroll-behavior: contain;
  - top: 88px (or consistent topbar offset)
- If using JS for fixed alignment, JS MUST also set:
  - maxHeight and overflowY to ensure internal scrolling always works.
STYLE RULES (REFERENCE LOCK)
Use the same design language and patterns as my reference UI:
- A top bar with brand mark + title + subtitle
- Optional right-side action chips only if PAGE_META provides actions
- Two-column layout: main content + fixed side panel
- Soft glassy surfaces, subtle shadows, rounded corners, Tailwind-ish tokens
- Section separators with a thin gradient rule
- Tables inside a scroll-safe wrapper with sticky headers
- Code blocks styled like the reference (rounded, bordered, readable)
- Overflow-safe: never clip the right edge, and never allow whole-page horizontal scroll
- Background uses subtle radial gradients like the reference
- Use CSS variables for slate scale + accents (blue/teal/violet/amber/rose/green)
- Use consistent class naming: page, bg, topbar, frame, layout, main, side-slot, side-fixed, card, stack, table-wrap, doc-table, pre, etc.
- Make sure the different cards are not joined together without some spacing
TEXT AND VOICE RULES
- Keep the page text clean and professional.
- Avoid the em dash character completely. Do not use -- or - anywhere.
- Avoid "not X but Y" sentence structures.
- Short, direct, punchy microcopy is fine.
- Do not add motivational filler unless the Markdown already implies it.
NO SURPRISE RULE
- Do not invent content.
- Do not rewrite meaning.
- Only structure and present what the Markdown contains.
- You can add tiny helper labels that are purely UI scaffolding (Jump list, Scan cues, Quick links).
CONTENT TRANSFORMATION RULES (MARKDOWN -&gt; STRUCTURED HTML)
You MUST parse the Markdown and render it as structured HTML using these rules:
1) Title
- If PAGE_META provides title, use it as the main page title.
- Else, use the first H1 in Markdown.
- Else, "Document".
2) Sections
- Every H2 becomes a major section card with:
  - A section header block: title + optional short subtitle (only if Markdown contains a subtitle line directly under the heading)
  - An anchor id derived from the heading text (kebab-case)
- H3 becomes a subsection inside the current section.
3) Lists
- Convert bullet lists to &lt;ul class="ul"&gt; with good spacing.
- Nested lists must be preserved and indented.
4) Tables
- Convert Markdown tables into:
  - &lt;div class="table-wrap"&gt;&lt;table class="doc-table"&gt;...&lt;/table&gt;&lt;/div&gt;
- Include &lt;thead&gt; and &lt;th&gt;.
- Sticky &lt;thead&gt;.
- Ensure long text wraps and never breaks layout:
  - table-layout: fixed;
  - th, td: overflow-wrap: anywhere; word-break: break-word;
5) Code blocks
- Convert fenced code blocks into:
  - optional small language label above the block
  - &lt;pre class="pre"&gt;&lt;code&gt;...&lt;/code&gt;&lt;/pre&gt;
- Escape HTML inside code blocks.
- Preserve indentation exactly.
6) Quotes
- Convert Markdown blockquotes into a callout card style.
7) Links
- Convert links to &lt;a&gt; using default link styling.
8) Images
- If Markdown includes images, render them as responsive images inside a rounded frame.
SIDE PANEL CONTENT (ALWAYS PRESENT)
The side panel must include these cards:
1) Jump list - Auto-generate links to each H2 section anchor.
2) Scan cues - Extract 6 to 10 keywords from headings, render as pills. Also bold key terms using &lt;mark class="kw"&gt;.
3) Quick links - If PAGE_META has links, list them. Else list up to 6 from Markdown.
TOP BAR REQUIREMENTS
Always include a top bar:
- Left: brand mark (2 letters derived from title), title, subtitle
- Right: action chips only if PAGE_META.actions exists
FOOTER REQUIREMENTS
Always include a footer:
- Left: title + subtitle
- Right: footer chips only if PAGE_META.footerLinks exists
RESPONSIVE RULES
- Desktop: 2-column grid, side fixed or sticky, side internal scroll.
- &lt;= 980px: collapse to 1 column, side becomes static below main.
- &lt;= 520px: tighten padding, stack grids, avoid overflow.
Note:
- Do not start the html with &lt;!doctype html&gt; since it's going to be previewed in markdown enhanced preview extension.
- Always include the fixed topbar CSS and main.frame padding-top: 50px.
- Always include the side-fixed CSS with max-height, overflow, and padding-top.
PAGE_META FORMAT (OPTIONAL)
PAGE_META:
title: ...
subtitle: ...
brandMark: ... (optional)
...
footerLinks:
  - label: ...
    href: ...
NOW FIND BELOW THE MARKDOWN CONTENT WHOSE PAGE YOU NEED TO DESIGN:
( When I paste the Markdown content, generate the full HTML page in one single html code block using every rule above. )
[MARKDOWN CONTENT BELOW]</textarea>
            </div>
          </section>
      ...
      <aside class="rail rail-right" id="railRight" aria-label="Right panel">
        <div class="rail-top">
          ...
          <div class="rail-sub">Beautify Markdown File</div>
        ...
        <div class="rail-body">
          <div class="panel-card">
            <div class="panel-head">
              ...
              <div class="panel-sub">Markdown → Tailwind-ish HTML page</div>
            </div>
            <div class="stack">
              <div class="stack-item">Paste any Markdown content at the end of this prompt</div>
              <div class="stack-item">The AI generates a complete, standalone HTML page matching the Caufero reference UI</div>
              <div class="stack-item">Output is one single HTML code block, copy-paste ready for FileMaker WebViewer</div>
              <div class="stack-item">Two-column layout with fixed side panel, responsive, overflow-safe</div>
            </div>
          </div>

          <div class="panel-card">
            <div class="panel-head">
              <div class="panel-title">Key Rule Categories</div>
              <div class="panel-sub">What the prompt enforces</div>
            </div>
            <div class="pill-row">
              <span class="pill2">Output format (strict)</span>
              <span class="pill2">Width + layout</span>
              <span class="pill2">Side panel (fixed scroll)</span>
              <span class="pill2">Style (reference lock)</span>
              <span class="pill2">Content transformation</span>
              <span class="pill2">No surprise rule</span>
              <span class="pill2">Responsive</span>
              <span class="pill2">Text + voice</span>
            </div>
          </div>
          ...
          <div class="panel-card">
            ...
            <div class="pill-row">
              <span class="pill2">Tailwind-ish</span>
              <span class="pill2">WebViewer</span>
              <span class="pill2">Two-column grid</span>
              <span class="pill2">Side panel</span>
              <span class="pill2">PAGE_META</span>
              <span class="pill2">Jump list</span>
              <span class="pill2">Scan cues</span>
              <span class="pill2">Overflow-safe</span>
              <span class="pill2">No blank lines</span>
            </div>
          </div>

          <div class="panel-card">
            <div class="panel-head">
              <div class="panel-title">Content Transformations</div>
              <div class="panel-sub">Markdown → HTML mappings</div>
            </div>
            <div class="side-grid">
              <span class="side-pill">H1 → Page title</span>
              <span class="side-pill">H2 → Section card with anchor</span>
              <span class="side-pill">H3 → Subsection</span>
              <span class="side-pill">Tables → scroll-safe wrapper</span>
              <span class="side-pill">Code blocks → pre + language label</span>
              <span class="side-pill">Blockquotes → callout card</span>
              <span class="side-pill">Lists → styled ul/ol</span>
              <span class="side-pill">Images → rounded responsive frame</span>
            </div>
          </div>
          ...
          <div class="panel-card">
            <div class="panel-head">
              ...
              <div class="panel-sub">Page generation prompts</div>
            </div>
            <div class="side-grid">
              <a class="side-pill" href="https://caufero.github.io/projects/_Misc/_/AI%20Prompts/Make%20A%20Page%20ChatGPT%20Generated%20Become%20Better.md">Make A Page Better</a>
              <a class="side-pill" href="https://caufero.github.io/projects/_Misc/_/AI%20Prompts/Convert%20Current%20CSS%20To%20Tailwind%20CSS.md">Convert CSS to Tailwind</a>
            </div>
          </div>
          ...
          <div class="panel-card">
            ...
            <div class="stack">
              ...
              <div class="stack-item">2. Optionally add a PAGE_META block for title, subtitle, actions</div>
              <div class="stack-item">3. Paste your raw Markdown content at the end</div>
              <div class="stack-item">4. Send to the AI and get a complete HTML page back</div>
              <div class="stack-item">5. Copy the HTML into your FileMaker WebViewer or .md preview</div>
            </div>
          </div>
        </div>
...
function filterNav(q){
  ...
  <style>
    ...
    *{box-sizing:border-box;}
    ...
    /* ---- Prompt textarea ---- */
    /* ---- Steps section ---- */
    .steps-body{padding:0 20px 20px 20px;}
    .steps-list{
      margin:0;
      padding:0 0 0 22px;
      list-style:decimal;
    ...
    .steps-tip{
      ...
      background:linear-gradient(180deg, rgba(34,197,94,.06), rgba(255,255,255,.70));
    }
    .tip-badge{
      flex:0 0 auto;
      font-size:11px;
    ...
    .page[data-theme="dark"] .tip-badge{color:rgba(74,222,128,1);}
    .tip-text{font-size:13px;color:var(--body-text);line-height:1.5;}
    .inline-code{
    ...
    .code-ref-wrap{padding:12px 16px 18px 16px;}
    .code-ref{
      margin:0;
      padding:14px 14px;
      border-radius:16px;
    ...
    .code-ref code{font-family:inherit;}
    .code-ref::selection,.code-ref *::selection{background:rgba(65,197,224,.35);}
    .page[data-theme="dark"] .code-ref{
      border-color:rgba(241,245,249,.10);
      background:linear-gradient(to bottom, rgba(2,6,23,.95), rgba(2,6,23,.90));
    }
